  ...
  if (currentUser) {
    ...
    setCurrentView('dashboard');
      ...
      useEffect(() => {
        ...
        // ðŸ”’ Wichtig für sichere Firestore Rules:
  ...
  try {
    ...
    showToast('Benachrichtigungen konnten nicht aktiviert werden');
      ...
      const deleteWorkClockSession = async (session) => {
        ...
        if (!window.confirm(`Session â€ž${label}â€œ löschen?`)) return;
...
const promptInstallPwa = async () => {
  try {
    if (!deferredInstallPrompt) {
      showToast(isIosUA ? 'iPhone/iPad: Teilen â†’ â€žZum Home-Bildschirmâ€œ installieren.' : 'Installation ist gerade nicht verfügbar.');
      return;
    }
...
const openEditEventModal = (event, occurrenceDate = null, opts = {}) => {
      ...
      async function findProfileByFriendCode(code5) {
        ...
        try {
          ...
          // Fallback: alte Profile hatten friendCode evtl. als Number â†’ suche über local allProfiles
                  ...
                  return (
                    <div className="bg-neutral-950 border-t border-neutral-800 px-4 md:px-8 py-4 shrink-0">
                      <div className="flex items-center justify-between mb-3">
                        ...
                        <div className="text-[11px] text-neutral-500">ðŸ—³ï¸ {openPollEvents.length} • ðŸ’¬ {commentEvents.length}</div>
                      </div>
                              ...
                              <div key={ev.id} className="border border-neutral-800 rounded-xl p-3 mb-2 last:mb-0">
                                      ...
                                      return (
                                        ...
                                        >
                                          <div className="flex items-center justify-between gap-2">
                                            <div className="text-xs text-white">{opt.date} • {opt.time}{isWinner ? ' â­' : ''}</div>
                                ...
                                <div className="mt-2 flex items-center justify-between">
                                  <div className="text-[11px] text-neutral-500">
                                    {myComplete ? 'Du: fertig âœ…' : 'Du: offen'}{deadlineLabel ? ` · Deadline: ${deadlineLabel}` : ''}
                                  </div>
                                  {canWrite && (
                                    <button
                                      type="button"
                ...
                <div className="bg-neutral-950/50 border border-neutral-800 rounded-xl p-5 space-y-4">
                  ...
                  <div className="flex items-start justify-between gap-4">
                    <div>
                      ...
                      <p className="text-xs text-neutral-500 mt-1">Standard-Erinnerung gilt für neue Termine (und für Termine mit â€žStandardâ€œ). Pro Termin kannst du es im Termin-Modal überschreiben.</p>
                    </div>
                    <div className="flex gap-2">
                  ...
                  <div className="border border-neutral-800 rounded-xl bg-black p-4">
                    ...
                    <div className="mt-3 space-y-3">
                      ...
                      <div className="text-[11px] text-neutral-600">
                        Service Worker: <span className="text-neutral-300">{pushDiag.sw}</span>{pushDiag.controlling ? <span className="text-neutral-500"> · controlling âœ…</span> : <span className="text-neutral-500"> · not controlling</span>}
                      </div>
  ...
  if (active.length === 0 && expired.length === 0) {
    return (
      <div className="text-sm text-neutral-500 border border-dashed border-neutral-800 rounded-xl p-4 text-center">
        Noch keine Links. Erstelle einen Link über <span className="text-neutral-300">Link erstellen</span> oder das <span className="text-neutral-300">ðŸ”’</span> Icon am Kalender.
      </div>
    );
  }
          ...
          <div className="mt-3 space-y-2">{expired.slice(0, 40).map(renderItem)}</div>
                      ...
                      <div className="mt-1 space-y-1">
                        ...
                        {aliasEditOpen && (
                          <div className="mt-3 border border-neutral-800 rounded-xl p-3 bg-black space-y-2">
                            <div className="text-xs text-neutral-500">Alias ändern (2-20 Zeichen). Erlaubt: a-z, 0-9, <span className="font-mono">._-</span>. Leerzeichen â†’ <span className="font-mono">_</span>.</div>
                            <input
                              value={aliasDraft}
                ...
                <div className="mt-4 bg-neutral-950/50 border border-neutral-800 rounded-xl p-5">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-neutral-500 mt-1">
                        {hasPasswordProvider()
                          ...
                          : 'Dieses Konto nutzt keinen Passwortâ€‘Login. Du kannst dir eine Resetâ€‘Mail senden, um ein Passwort zu setzen/ändern.'}
                      </p>
                    </div>
                  ...
                  {hasPasswordProvider() && pwEditOpen && (
                    <div className="mt-4 space-y-3">
                      ...
                      <div className="text-[11px] text-neutral-600">
                        Tipp: Wenn du dein aktuelles Passwort nicht mehr weißt, nutze â€žResetâ€‘Mailâ€œ.
                      </div>
                    </div>
                  )}
                </div>
            ...
            {q && filteredTabs.length === 0 && (
              <div className="border border-neutral-800 rounded-2xl p-6 bg-neutral-950/50 text-sm text-neutral-500">
                Keine Treffer für â€ž{settingsQuery}â€œ.
              </div>
            )}
          </main>
        </div>
                    ...
                    <div className="flex-1 overflow-y-auto p-6 max-w-2xl w-full mx-auto space-y-8">
                      <div className="flex flex-col items-center border border-neutral-800 rounded-xl p-8 bg-neutral-950/50">
                        ...
                        <form onSubmit={updateProfileSettings} className="w-full max-w-xs space-y-4">
                          ...
                          <p className="text-[11px] text-neutral-500 text-center">Freunde im Chat fügst du über diese Chatâ€‘ID hinzu. Kalenderâ€‘Freunde werden per Eâ€‘Mail geteilt.</p>
                      ...
                      {createPollOnSave && (
                        <div className="mt-3 space-y-3">
                          ...
                          <div className="mt-2 bg-neutral-950/40 border border-neutral-800 rounded-xl p-3">
                            ...
                            <div className="mt-2 text-[11px] text-neutral-500">Voting 2.0: âœ… Fix · ðŸ¤· Kann · âŒ Nein (pro Vorschlag)</div>
                          </div>
<div className="flex gap-2">
                            <button
                              type="button"
        ...
        {(!modalEvent || !modalEvent.poll || modalEvent.poll.status !== 'open') ? (
          <div className="space-y-3">
            <p className="text-xs text-neutral-400">
              3 Vorschläge senden â†’ Mitglieder mit <span className="text-white">Schreibrecht</span> im Kalender stimmen ab. Gewinner wird automatisch gesetzt.
            </p>
                    ...
                    {(poll.options || []).map((opt) => {
                      ...
                      if (tally.version >= 2) {
                        ...
                        return (
                          ...
                          >
                            <div className="flex items-center justify-between gap-2">
                              <div className="text-sm text-white">{opt.date} • {opt.time}{isWinner ? ' â­' : ''}</div>
                              <div className="flex items-center gap-1">
              ...
              <div className="bg-neutral-950 border border-neutral-800 w-full sm:max-w-lg rounded-t-2xl sm:rounded-2xl p-5 shadow-2xl animate-slide-up" onClick={(e) => e.stopPropagation()}>
                ...
                <form onSubmit={saveCalendarSettings} className="space-y-4">
                  ...
                  <div>
                    ...
                    <p className="mt-1 text-[11px] text-neutral-500">Schichtplan: Tippen wechselt Schichten, lang drücken öffnet Auswahl, â€žPinselâ€œ malt mehrere Tage.</p>
                  </div>
              ...
              <div className="bg-neutral-950 border border-neutral-800 w-full sm:max-w-lg rounded-t-2xl sm:rounded-2xl p-5 shadow-2xl animate-slide-up" onClick={(e) => e.stopPropagation()}>
                ...
                {shareLinkCreated && (
                  <div className="mt-4 bg-black border border-neutral-800 rounded-xl p-4">
                    ...
                    <div className="mt-2 text-[11px] text-neutral-500">Hinweis: Externe Links benötigen passende Firestoreâ€‘Rules für <span className="font-mono text-neutral-300">shares</span>.</div>
                  </div>
                )}
                      ...
                      <div className="space-y-2 max-h-[32vh] overflow-y-auto no-scrollbar pr-1">
                        {relevant.slice(0, 20).map(l => {
                          ...
                          return (
                            <div key={l.id} className="flex items-center justify-between bg-black border border-neutral-800 rounded-xl px-4 py-3">
                              <div className="min-w-0">
                                <div className="text-sm text-white font-medium truncate">{l.kind === 'calendar' ? 'Busyâ€‘Only' : 'Event'} • {shareLinkDraft.calName}</div>
