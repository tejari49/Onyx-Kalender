import React, { useState, useEffect, useRef } from 'react';
    ...
    const DEFAULT_WEATHER_LOCATION = { name: 'OberbÃ¼ren, SG', lat: 47.45, lon: 9.11 };
    ...
    showToast('Benachrichtigungen konnten nicht aktiviert werden');
      ...
      const addShoppingPurchase = async (listId) => {
        ...
        if (!Number.isFinite(amount) || amount <= 0) {
          showToast('Bitte einen gÃ¼ltigen Betrag eingeben');
          return;
        }
      ...
      function getWeatherHintForTimestamp(ts) {
        try {
          ...
          if (rainProb >= 55 && wind >= 30) return ' â€¢ Wetter: Regen + Wind â€“ Schirm/Regenjacke';
          if (rainProb >= 55) return ' â€¢ Wetter: Regen mÃ¶glich â€“ Schirm mitnehmen';
          if (wind >= 35) return ' â€¢ Wetter: windig â€“ Jacke einplanen';
          return '';
        } catch (_) {
          return '';
        }
    ...
    for (const ev of list) {
      ...
      const fireReminderIfDue = async (item) => {
        try {
          ...
          const body = `${item.title}${item.time ? ' â€¢ ' + item.time : ''} â€¢ ${calName}${weatherHint}`;
            ...
            <div className="shrink-0 flex justify-end px-3 md:px-4 pt-3 md:pt-4">
              ...
              <div className="p-5 md:p-10 max-w-6xl w-full mx-auto animate-fade-in space-y-6">
                ...
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4">
                  ...
                  <div className="border border-neutral-800 rounded-2xl bg-neutral-950/50 p-4">
                    <div className="text-[10px] uppercase tracking-[0.22em] text-neutral-500">Chats</div>
                    <div className="mt-2 text-sm font-semibold text-white">{hasUnreadMessages ? 'Neue Nachrichten' : 'Alles gelesen'}</div>
                    <div className="mt-1 text-xs text-neutral-500">{hasUnreadMessages ? 'Kalender Aktuell aktiv' : 'Keine offenen Chats'}</div>
                  </div>
                </div>
                            ...
                            {normalizeShoppingPurchases(activeShoppingList.purchases).slice(0, 5).map((row) => (
                              <div key={row.id} className="flex items-center justify-between text-xs">
                                <span className="text-neutral-400 truncate pr-3">
                                  {(row.store || activeShoppingList.store || 'Ohne Laden')} Â· {new Date(row.paidAt).toLocaleDateString('de-CH')}{row.note ? ` Â· ${row.note}` : ''}
                                </span>
                                <span className="text-neutral-200">{formatCurrencyCHF(row.amount)}</span>
                              </div>
                            ))}
...
- ...`} className="w-full bg-black border border-neutral-800 rounded-xl px-4 py-3 text-sm text-white focus:outline-none focus:border-neutral-500" />
    ...
    const SETTINGS_SECTIONS = [
      ...
      { id: 'notifications', label: 'Produktivität & Push', icon: Activity, keys: ['extras', 'smart day', 'stempelung', 'fokus', 'notiz', 'tagesziel', 'soll ist', 'freie zeitfenster', 'push', 'benachr', 'reminder', 'erinnerung', 'ton', 'pwa', 'token', 'test'] },
              ...
              const AccordionItem = ({ id, children }) => {
                ...
                return (
                  <div className="border border-neutral-800 rounded-2xl lg:rounded-3xl overflow-hidden bg-neutral-950/50">
                    <button type="button" onClick={() => { if (!q) setSettingsTab(id); }} className="w-full flex items-center justify-between px-4 py-3 lg:px-5 lg:py-4 bg-neutral-950 hover:bg-neutral-900 transition-colors">
                      <div className="flex items-center gap-3">
                ...
                <div className="mt-4 bg-neutral-950/50 border border-neutral-800 rounded-xl p-5">
                  <div className="flex items-start justify-between gap-4">
                    <div>
                      ...
                      <p className="text-xs text-neutral-500 mt-1">WÃ¤hle zwischen Dunkel und einem weichen hellen Modus.</p>
                    </div>
                    <div className="flex gap-2">
                      <button
                        type="button"
